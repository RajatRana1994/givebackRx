...
class AppUtils @Inject constructor(private val mContext: Context) {
    ...
    fun getLocaleLanguage(): String {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            lang = Resources.getSystem().getConfiguration().getLocales().get(0).language
        } else {
            lang = Resources.getSystem().getConfiguration().locale.language
        }
...
//        window.statusBarColor = ContextCompat.getColor(activity, R.color.transparent)
    ...
    public fun convertDateFormat(date: String, from: String, to: String): String {
    ...
    fun getUserData(mPrefs: PreferenceHelper): SignInWithUserDetailData? {
        try {
            val gson = Gson()
            return gson.fromJson(
                mPrefs.getKeyValue(PreferenceConstants.USER_DATA),
                SignInWithUserDetailData::class.java
            )
        } catch (e: Exception) {
            return null
        }
