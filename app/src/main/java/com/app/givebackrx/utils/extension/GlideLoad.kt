...
internal fun ImageView.loadImageRadius(img:Any,radius:Int=20){
    when(img){
        is String, is Int, is Uri->{
            Glide.with(this.context).load(img).apply(RequestOptions().transform(RoundedCorners(radius)).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher)).into(this)
        }
    }
}
...
internal fun ImageView.loadUserImage(img:Any){
    when(img){
        is String, is Int, is Uri->{
            Glide.with(this.context).load(img).apply(RequestOptions.circleCropTransform().error(R.mipmap.ic_launcher)).into(this)
        }
    }
}
...
internal fun ImageView.loadOrigImage(img:Any){
    when(img){
        is String, is Int, is Uri->{
            Glide.with(this.context).load(img).apply(RequestOptions().error(R.mipmap.ic_launcher)).into(this)
        }
    }
}
...
internal fun ImageView.makeGrey(){
    ...
    this.setColorFilter(ColorMatrixColorFilter(matrix))
}
